...
while True:
    ...
    if (sensor_left & sensor_center & sensor_right):
        ...
        mod = 1
    # center sensor over black line
    # straight
    elif ((not sensor_left) & sensor_center & (not sensor_right)):
        display.show(Image.ARROW_N)
        moto.drive_stop(speed, speed, time)
        mod = 1
    # left & right sensor over black line
    # straight
    elif (sensor_left & (not sensor_center) & sensor_right):
        display.show(Image.ARROW_N)
        moto.drive_stop(speed, speed, time)
        mod = 1
